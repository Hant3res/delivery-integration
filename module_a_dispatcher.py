...
def assign_delivery():
    ...
    # Find available courier (available = True)
    courier = db.query(Courier).filter(Courier.available == True).first()
    
    if not courier:
        db.close()
        logger.warning(f"No available couriers for order {order_id}")
    ...
    logger.info(f"Delivery assigned: {task_id} to courier {courier_name}")
